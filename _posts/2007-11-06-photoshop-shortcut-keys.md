...
+ New - Ctrl(Cmd) + N , Ctrl + 바탕더블클릭 : 새 종이를 불러옵니다
+ Open - Ctrl(Cmd) + O(영문) , 바탕 더블 클릭 : 이미지를 불러옵니다
+ Browse in Bridge - shift + Ctrl(Cmd) + O(영문) , Alt(Opt) + Ctrl(Cmd) + O(영문) : Bridge열기
+ Open As -  Alt(Opt) + shift+ Ctrl(Cmd) + O(영문)
...
+ 텍스트를 친 후 자간 조절하고자 하는 내용에 블럭설정을 하고 ctrl+alt 화살표(←,→)키로 조절하면 조절폭이 커지고 alt 화살표(←,→)키로 조절하면 조절폭이 작아집니다.
