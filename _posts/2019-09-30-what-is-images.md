...
디지털 이미지(Disital Images)는 디지털 카메라에서처럼 원본 데이터를 이미지 센서에서 표준 이미지 파일 포맷의 색깔 수정 이미지로 변환하는 전용 디지털 영상 처리 프로세서를 필요로 합니다. 이러한 이유로 컴퓨터상에서 이미지(또는 이미지파일)는 래스터 그래픽(Raster graphics) 형태의 픽셀(Pixels)단위의 정보를 갖습니다.
